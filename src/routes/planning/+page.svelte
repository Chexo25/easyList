<script lang="ts">
  ...
  import { ShoppingCart, ChevronLeft, ChevronRight, Plus, Trash2, CalendarDays, Utensils, Check, X } from 'lucide-svelte';
  ...
  async function handleMealSelect(mealId: string | null) {
    if (selectionTarget) {
      if (selectionTarget.type === 'lunch') {
          await updatePlan(selectionTarget.date, { lunch: mealId, lunchExcluded: [] });
      } else {
          await updatePlan(selectionTarget.date, { dinner: mealId, dinnerExcluded: [] });
      }
    }
    mealSelectionOpen = false;
  }

  // --- DETAILS INGREDIENTS D'UN REPAS PLANIFIÉ ---
  ...
  async function toggleIngredientExclusion(ingName: string) {
     ...
     let nextEx: string[];
     if(detailsTarget.excluded.includes(ingName)) {
        nextEx = detailsTarget.excluded.filter(n => n !== ingName);
     } else {
        nextEx = [...detailsTarget.excluded, ingName];
     }
        ...
        for (const ming of meal.ingredients) {
            ...
            if (activeIndex !== -1) {
              ...
              if (boughtIndex !== -1) {
                ...
                toSave[boughtIndex] = { ...exist, isBought: false, quantity: ming.quantity || 1, linkedMeals: [meal.name], category: ming.category, unit: ming.unit || exist.unit };
              } else {
                toSave.push({
                  id: uuidv4(),
                  name: ming.name,
...
<div class="flex flex-col h-full min-h-0 w-full bg-background">
  ...
  {#if activeTab === 'day'}
    ...
    <div class="flex-1 overflow-y-auto p-4 flex flex-col gap-6">
      <!-- Sélecteur de jour étendu et centré -->
...
<!-- Modal Détails Repas (Ingrédients à exclure) -->
    ...
    {#if detailsTarget}
      ...
      <div class="flex-1 overflow-y-auto p-4 space-y-2">
        {#each detailsTarget.meal.ingredients as ing}
          {@const cx = !detailsTarget.excluded.includes(ing.name)}
          <label class="flex items-center justify-between p-3 border rounded-xl cursor-pointer hover:bg-muted/50 transition-colors {cx ? 'bg-card' : 'bg-muted/30 opacity-70'}">
            <div class="flex items-center gap-3">
              <input 
                type="checkbox" 
                class="w-5 h-5 rounded border-2 border-muted-foreground cursor-pointer focus:ring-0 focus:ring-offset-0 text-foreground accent-neutral-700" 
                checked={cx} 
                onchange={() => toggleIngredientExclusion(ing.name)}
              />
              <span class="font-medium {cx ? '' : 'line-through text-muted-foreground'}">{ing.name}</span>
            </div>
            {#if ing.quantity}
...
<!-- Modal Export Courses -->
<Dialog.Root bind:open={exportDialogOpen}>
  <Dialog.Content class="max-w-[400px] w-[92vw] max-h-[85vh] p-0 flex flex-col rounded-2xl overflow-hidden">
    <Dialog.Header class="p-6 pb-4 shrink-0 border-b">
      <Dialog.Title class="text-xl">Ajouter aux courses</Dialog.Title>
      <Dialog.Description class="mt-2 text-left">
        Sélectionnez les repas de la semaine que vous souhaitez acheter. (Les ingrédients décochés individuellement sont bien ignorés).
      </Dialog.Description>
    </Dialog.Header>
      ...
      {:else}
        {#each toExport as exp (exp.id)}
          <label class="flex items-center gap-4 py-3 px-4 mx-2 my-1 border bg-card rounded-xl cursor-pointer hover:bg-muted transition-all {exp.selected ? 'border-primary/20' : 'opacity-60'}">
            <input 
               type="checkbox" 
               class="w-5 h-5 rounded border-2 border-muted-foreground text-foreground accent-neutral-700 focus:ring-0" 
               bind:checked={exp.selected} 
            />
            <div class="flex-1 min-w-0">
