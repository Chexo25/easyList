  <script lang="ts">
    ...
    let planning = $derived(
      Object.fromEntries(
        Object.entries($syncPlanning || {}).map(([key, val]) => [
          key,
          {
            ...val,
            lunchExcluded: val.lunchExcluded || [],
            dinnerExcluded: val.dinnerExcluded || [],
          }
        ])
      ) as Planning
    );
    ...
    async function handleMealSelect(mealId: string | null) {
      if (selectionTarget) {
        ...
        await updatePlannedDayInSync(selectionTarget.date, updates);
      }
    ...
    async function toggleIngredientExclusion(ingName: string) {
      ...
      await updatePlannedDayInSync(detailsTarget.date, updates);
    }
      ...
      for (const d of weekDays) {
        ...
        if (plan) {
          if (plan.lunch) {
            ...
            if (m) list.push({ id: crypto.randomUUID(), date: str, label: 'Midi', meal: m, selected: true, excluded: plan.lunchExcluded || [] });
          }
          if (plan.dinner) {
            const m = meals.find(x => x.id === plan.dinner);
            if (m) list.push({ id: crypto.randomUUID(), date: str, label: 'Soir', meal: m, selected: true, excluded: plan.dinnerExcluded || [] });
          }
        }
      }
      toExport = list;
              ...
              <div class="border rounded-xl p-0 shadow-sm bg-card hover:bg-muted/30 transition-colors flex flex-col overflow-hidden">
                <button type="button" class="w-full p-4 flex justify-between items-center cursor-pointer text-left" onclick={() => openDetails(formatDate(activeDate), 'lunch', meal)}>
                  ...
                  <div class="flex space-x-2 ml-4 shrink-0" onclick={(e) => e.stopPropagation()} role="presentation">
                    ...
                    <Button variant="outline" size="icon" class="h-8 w-8 text-destructive border-red-100 bg-red-50 hover:bg-red-100" onclick={() => updatePlannedDayInSync(formatDate(activeDate), { lunch: null, lunchExcluded: [] })}>
              ...
              <div class="border rounded-xl p-0 shadow-sm bg-card hover:bg-muted/30 transition-colors flex flex-col overflow-hidden">
                <button type="button" class="w-full p-4 flex justify-between items-center cursor-pointer text-left" onclick={() => openDetails(formatDate(activeDate), 'dinner', meal)}>
                  ...
                  <div class="flex space-x-2 ml-4 shrink-0" onclick={(e) => e.stopPropagation()} role="presentation">
                    ...
                    <Button variant="outline" size="icon" class="h-8 w-8 text-destructive border-red-100 bg-red-50 hover:bg-red-100" onclick={() => updatePlannedDayInSync(formatDate(activeDate), { dinner: null, dinnerExcluded: [] })}>
                      <Trash2 class="w-4 h-4" />
                    </Button>
                  </div>
                </button>
